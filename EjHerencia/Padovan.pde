...
class Padovan extends Sequence {
  ...
  void display(int n) {
    ...
    float base = height/(n*(n/4));
    for(int i=0;i<n; i++){
    xa[0]=width/2;
    ya[0]=height/2;
    xb[0]=xa[0]+base/*float(base)*//2;
    yb[0]=ya[0]-sqrt(3)*base/*float(base)*//2;
    xc[0]=xa[0]+base/*float(base)*/;
    yc[0]=ya[0];
    xa[1]=xc[0];
    ya[1]=yc[0];
    xb[1]=xa[1]-/*float(base)*/base/2;
    yb[1]=ya[1]+sqrt(3)*base/*float(base)*//2;
    xc[1]=xa[1]-base/*float(base)*/;
    yc[1]=ya[1];
    xa[2]=xc[1];
    ya[2]=yc[1];
    xb[2]=xa[2]+base/*float(base)*//2;
    yb[2]=ya[2]+sqrt(3)*base/*float(base)*//2;
    xc[2]=xb[2]-base/*float(base)*/;
    yc[2]=yb[2];
    if(i>2){
      xa[i]=xc[i-1];
      ya[i]=yc[i-1];
      ...
      radius=int(base)*compute(i+1);
      xc[i]=xa[i]+(radius*cos(angle));
      yc[i]=ya[i]-(radius*sin(angle));      
    }
     fill(0, 100, map(compute(i+1), 0, compute(n), 255, 0));
